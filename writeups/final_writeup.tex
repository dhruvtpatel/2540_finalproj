...
Ultimately, we aim to evaluate the \textbf{feasibility of automatically generating and verifying equivalent early assertions in Python programs} across a diverse set of real-world examples. While conducting FEQ evaluartions, we also identify \textbf{\textit{false positives}} where \textbf{assertions are deemed not equal due to a non-logical error} with the method, an unsupported feature, type errors, etc. In this case, the assertion equivalence for that method would be deemed inconclusive. 

% Success is measured not only by \textbf{correctness and soundness} (through symbolic execution, fuzz testing, and static verification), but also by the method's \textbf{ability to scale} to programs of varying size, complexity, and structure—while maintaining \textbf{low false positive rates and manageable performance overhead.}
...
We evaluated the pipeline on 50 Python functions, each implementing basic arithmetic, complex control flow, or structural operations such as multiplication, discount calculation, factorial calculation, sine function, and data transformation. These functions span operations from basic mathematical computations to complex string processing and data transformations. 

We created a variety of automated scripts and calculated difficulty, ran testing and verification, failure explanation scores, etc. for every program. In particular, we \textbf{identified Nagini-unsupported programs using GPT-4.1} (with Nagini's wiki as context) to assess compatibility with its specification language, followed by manual verification \cite{eilers2018}. We also used a similar LLM-as-a-judge approach to score failures across all methods to calculate Failure Explanation Quality (FEQ) scores and false positives. 
...
collatz\_sequence\_length & 7 & pass & pass & pass & 0.57 (crosshair) \\
convert\_temperature & 6 & inconclusive (false positive) & fail & unsupported & 0.74 (crosshair) \\
...
isbn\_validator & 9 & inconclusive (false positive) & pass & unsupported & 0.50 (fuzz) \\
...
matrix\_determinant & 7 & inconclusive (false positive) & pass & unsupported & 0.53 (fuzz) \\
