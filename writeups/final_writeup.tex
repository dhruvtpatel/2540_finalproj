...
\subsection{Weakest Pre-Condition Predicate Transformation}

Recent work has adapted weakest precondition (WP) reasoning to dynamically typed languages, notably Python. \textbf{Rak-amnouykit et al.~\cite{rak-amnouykit2023weakest}} perform interprocedural WP analysis to infer input constraints that prevent runtime exceptions (e.g., \texttt{raise} statements). Applied to libraries like \texttt{scikit-learn}, the tool synthesized constraints such as \texttt{penalty="elasticnet"} $\Rightarrow$ \texttt{0 ≤ l1\_ratio ≤ 1}, matching or strengthening undocumented requirements. \textbf{Rak-amnouykit et al.~\cite{baudart2022raise}} extend this work to generate machine-checkable schemas and incorporate partial evaluation to simplify complex constraint expressions during analysis.

Other approaches apply WP-style techniques to interface specifications. \textbf{Cosler et al.~\cite{cosler2023nl2spec}} integrate backward propagation with large language models to synthesize temporal logic specifications from natural language API documentation. \textbf{Hassan et al.~\cite{xu2018typpete}} apply MaxSMT-driven WP reasoning for type inference in Python, using interprocedural abstract interpretation to generate precise contracts across function boundaries.

While WP analyses can infer sufficient conditions for correctness, they often struggle with dynamic constructs, mutable state, and aliasing~\cite{guarnieri2024approximate}. Our approach, by contrast, verifies that early, developer-specified assertions imply safety across all paths. This forward, path-sensitive reasoning avoids the synthesis overhead of WP and better integrates with assertion-based development workflows, providing stronger guarantees for correctness in real-world dynamic code.
...
Our goal is to proactively enforce security and correctness properties (and prevent unnecessary computation) in Python programs by shifting runtime assertion checks earlier in execution, while ensuring that the new early assertions are \textit{logically equivalent} to the original final assertions. To achieve this, we design a three-phase pipeline that combines large language models (LLMs), program transformation, and formal verification methods. This methodology allows us to generate proposal transformations and validate their accuracy.
...
% % \textbf{NEED TO REDO, THIS SECTION'S JUST YAP RIGHT NOW, INCLUDE STATISTICAL RELATIONSHIPS, DEEPER ANALYSIS OF PROGRAMS}

% \subsection{Execution Results} 
% % \textbf{NEED TO DO A BETTER BREAKDOWN IT'S NOT USEFUL RIGHT NOW, TOO HAND WAVY ATM}
...
Verification success rates are strongly tied to program complexity. As shown in Figure \ref{fig:diff-scores-correlation}, functions with overall difficulty scores below 5.0 passed CrossHair over 80\% of the time and Nagini over 40\%, while those with scores above 8.0 rarely succeeded under either tool. The average difficulty of Verified functions was 4.7, compared to 6.1 for Rejected ones (Table \ref{tab:difficulty-sub-scores}).
...
% \textbf{[need to be more specific here, could be in discussion]}

% Despite the strengths of our pipeline, a significant fraction of real-world Python code remains outside the reach of current formal and symbolic verification tools. As detailed in Table \ref{tab:nagini-unsupported-programs}, the most common unsupported features are floating-point arithmetic (80\% of unsupported cases), list comprehensions (65\%), randomness (40\%), and datetime or external library calls (30\%). These features either introduce non-determinism, require complex heap modeling, or depend on external semantics that are not captured by the verification engines. As a result, they define the current "blind spots" in automated assertion verification, and represent key targets for future tool development. Addressing these gaps will be essential for scaling formal security guarantees to the full diversity of Python programs encountered in practice.
...
\section{Discussion [NEED TO REDO, MAKE MORE RELEVANT, NEEDS TO BE TIED TO LIT A BIT]}
...
\bibitem{cosler2023nl2spec} Cosler, Matthias, Christopher Hahn, Daniel Mendoza, Frederik Schmitt, and Caroline Trippel. "nl2spec: Interactively Translating Unstructured Natural Language to Temporal Logics with Large Language Models." \textit{arXiv preprint arXiv:2303.04864}, 2023. \url{https://arxiv.org/abs/2303.04864}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{verification_by_difficulty.png}
...
\hline
\textbf{Method} & \textbf{Parameter} & \textbf{Operation} & \textbf{Control Flow} & \textbf{Data Type} & \textbf{Assertion} & \textbf{Math} & \textbf{Loop} & \textbf{Call} & \textbf{Branch} \\
 & \textbf{Complexity} & \textbf{Density} & \textbf{Depth} & \textbf{Diversity} & \textbf{Complexity} & \textbf{Sophistication} & \textbf{Complexity} & \textbf{Density} & \textbf{Complexity} \\
\hline
Fuzz Testing & 0.02 & -0.16 & -0.38 & -0.37 & -0.09 & 0.11 & NA & NA & NA \\
\hline
